...
Contracts.prototype.get_address = function(addr) {
    return this.lottery.methods.get_address(addr).call();
}


Contracts.prototype.approve = function(spender, value, privKey) {
    return new Promise((resolve, reject) => {
	const abi = this.ledger.methods.approve(spender, value).encodeABI();
	// TODO: verify max gas cost and adjust gas
	this.web3.eth.accounts.signTransaction(
	    {to: this.ledgerAddr, data: abi, gas: 100000}, privKey).then(function(signedTx) {
		this.web3.eth.sendSignedTransaction(signedTx.rawTransaction).then(function(res) {
		    resolve(res);
		});
	    });
    });
...
Contracts.prototype.fund = function(signer, amount, total, privKey) {
    return new Promise((resolve, reject) => {
	const abi = this.lottery.methods.fund(signer, amount, total).encodeABI();
	// TODO: verify max gas cost and adjust gas
	this.web3.eth.accounts.signTransaction(
	    {to: this.lotteryAddr, data: abi, gas: 100000}, privKey).then(function(signedTx) {
		this.web3.eth.sendSignedTransaction(signedTx.rawTransaction).then(function(res) {
		    resolve(res);
		});
	    });
...
Contracts.prototype.grabTicket = function(ticket, secret_rand, recPrivkey) 
{
    var c = this;
    // TODO: error handling
    return new Promise((resolve, reject) => 
    {
        ...
        // function grab(uint256 secret, bytes32 secret_hash, address payable target, uint256 nonce, uint256 ratio, uint64 amount, uint8 v, bytes32 r, bytes32 s) public {

        const winProb   = new BigNumber(ticket.winProb);
        const nonce     = c.web3.utils.hexToNumber(ticket.nonce);

        const hash  = this.web3.utils.soliditySha3(ticket.randHash, ticket.recipient, ticket.faceValue, winProb, nonce);
        const sig   = ticket.creatorSig;
        const sender_addr = this.web3.eth.accounts.recover(hash, sig.v, sig.r, sig.s);
        
        console.log("grabTicket  hash: + " + hash + " ticket.creatorAddr: " + ticket.creatorAddr + " sender_addr: " + sender_addr);


	    const abi = this.lottery.methods.grab(
	                    secret_rand, // secret from recipient
				        ticket.randHash,
				        ticket.recipient,
				        nonce,
				        winProb,
	    ...
	    c.web3.eth.accounts.signTransaction(
	        ...
	        {to: c.lotteryAddr, data: abi, gas: 200000},
	        // TODO: check which key is actually sending this tx
	        recPrivkey).then(function(signedTx) { c.web3.eth.sendSignedTransaction(signedTx.rawTransaction).then(function(res) {resolve(res);}); } );
...
Contracts.prototype.newTicket2 = function(secret_hash, faceValue, winProb, recipientAddr, creatorAddr, creatorPrivkey) {
    ...
    const hash  = this.web3.utils.soliditySha3(secret_hash, recipientAddr, faceValue, winProb, nonce);
    const sig   = this.web3.eth.accounts.signRaw(hash, creatorPrivkey);

    const creatorAddr2 = this.web3.eth.accounts.recover(hash, sig.v, sig.r, sig.s);
    
    console.log("newTicket2()  creatorAddr: " + creatorAddr + " " + creatorAddr2 );

    // TODO: encoding
    return {
        randHash:       secret_hash,
...
Contracts.prototype.doVerifyTicket = function(ticket,
    ...
    console.log("doVerifyTicket expectedWinProb: " + expectedWinProb + " ticketWinProb: " + ticketWinProb  );
...
Contracts.prototype.claimTicket = function(ticket, randNum, recipientPrivkey) {
    ...
    console.log("c.claimTicket 3  hash: " + hash);


    /*
	const ticketHash = c.web3.utils.soliditySha3(
	                       ticket.randHash,
	                       ticket.recipient,
					       ticket.faceValue,
					       new BigNumber(ticket.winProb),
    ...
    console.log("c.claimTicket 3b  ticketHash: " + ticketHash + " ticket.recipient: " + ticket.recipient + " ticket.faceValue: " + ticket.faceValue + " ticket.winProb: " + ticket.winProb + " ticket.nonce: " + ticket.nonce  );

	//const sig = c.web3.eth.accounts.signRaw(hash, recipientPrivkey);
	const sig = c.web3.eth.accounts.signRaw(ticketHash, recipientPrivkey);
	//const sig = c.web3.eth.accounts.sign(ticketHash, recipientPrivkey);

    console.log("c.claimTicket 4  sig: " + sig + " recipientPrivkey: " + recipientPrivkey);
    //console.log("c.claimTicket 4  ");
    

    const raddr1 = c.web3.eth.accounts.recover(ticketHash, sig.v, sig.r, sig.s);       
    console.log("c.claimTicket 4b  raddr1: " + raddr1 + " ticket.recipient: " + ticket.recipient);

    const raddr2 = c.web3.eth.accounts.recover(sig);       
    console.log("c.claimTicket 4c  raddr2: " + raddr2 + " ticket.recipient: " + ticket.recipient);

    const saddr1 = c.web3.eth.accounts.recover(ticketHash, ticket.creatorSig.v, ticket.creatorSig.r, ticket.creatorSig.s);       
    console.log("c.claimTicket 4d  saddr1: " + saddr1 + " ticket.creatorAddr: " + ticket.creatorAddr);
...
Contracts.prototype.doClaimTicket = function(randNum, randHash,
					   nonce, faceValue, winProb, sender, recipient,
					     v1, r1, s1, v2, r2, s2) {
